...
namespace Elite_Admin.Utility
{
    public class Logging
    {
        ...
        public static void WriteEmailLog(LogType logType, string text)
        {
            try
            {
                string LogPath = ConfigurationManager.AppSettings["EmailLogPath"];
                string file = AppDomain.CurrentDomain.BaseDirectory + LogPath;
                string dir = file.Substring(0, file.LastIndexOf(@"\"));
                if (Directory.Exists(dir) == false)
                    Directory.CreateDirectory(dir);
        ...
        public static void WriteSyncDataLog(LogType logType, string text)
        {
            try
            {
